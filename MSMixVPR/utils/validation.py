...
def get_validation_recalls(r_list, q_list, k_values, gt, print_results=True, faiss_gpu=False, dataset_name='dataset without name ?'):
        ...
        similarities, predictions = faiss_index.search(q_list, max(k_values))
        print("SIM:", similarities[0])
        ...
        for q_idx, pred in enumerate(predictions):
            for i, n in enumerate(k_values):
                ...
                if (dataset_name == 'synthia'):
                    if np.any(np.in1d(pred[:n], gt[q_idx][1])):
                        correct_at_k[i:] += 1
                        break
                else:
        ...
        print("CORRECT AT K:", correct_at_k)
